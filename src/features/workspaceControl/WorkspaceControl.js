...
export function WorkspaceControl() {
  ...
  const changeHandler = (event, field) => {
    ...
    if (field === "url") setUrl(event.target.value);
  };
  ...
  const submit = async (event) => {
    dispatch(
      postFile({
        name: name,
        srcstring: url,
        workspaceId: 1 /* temporary hardcode of workspaceId */,
      })
    );
  ...
  return (
    <div style={{ borderTop: "7px solid #68939c" }} className={classes.control}>
      <Typography className={classes.typography}>Workspace Controls</Typography>
      <div>
        ...
        >
          <Typography className={classes.typography}>
            ...
            <TextField
              variant="outlined"
              placeholder="url"
              onChange={(event) => changeHandler(event, "url")}
            />
      ...
      <List>
        {files.currentGroup
          ? files.currentGroup.map((file) => (
              <ListItem
                key={`list_item_${file.id}`}
                divider={true}
                onClick={(e) => fileClickHandler(file)}
                className={
                  files.currentFile.id === file.id ? classes.selectedFile : null
                }
              >
                {editing === file.id ? (
                  <TextField
                ...
                {editing === file.id ? (
                  <CheckCircleIcon onClick={(e) => saveClickHandler(file)} />
                ) : (
                  <EditIcon onClick={(e) => editClickHandler(file)} />
                )}
