...
export function WorkspaceSetup() {
  ...
  const handleUserChange = (e, user) => {
    if (e.target.checked) {
      if (
        workspaces.users_in_workspaces.filter(
          (object) =>
            object.userId === user.id && object.workspaceId === room.id
        ).length === 0
      ) {
  ...
  if (user.workspaces && !room) {
        ...
        <List>
          ...
          {user.workspaces.map((workspace) => (
            <ListItem>
              ...
              />
              <RemoveCircle
                onClick={(e) => deleteWorkspaceHandler(workspace.id)}
              />
            </ListItem>
          ))}
        </List>
    ...
    return (
      <div className={classes.workspaceSetup}>
        <TextField
          value={workspaceName}
          onChange={(e) => nameChangeHandler(e, room)}
        />
        <div style={{ display: "flex" }}>
          <div>
            <h3>FILES</h3>
            <Button color="primary" onClick={(e) => addFileHandler(e)}>
              {" "}
              Add a file
            </Button>
            <Popover
            ...
            <h5>Current files:</h5>
            {user.files
              .filter((file) => file.workspaceId === room.id)
              .map((file) => (
                <ListItem>
          ...
          <div>
            ...
            >
              ...
              <FormGroup>
                {user.all
                  .filter((person) => user.selectedUser !== person.id)
                  .map((user) => (
                    <FormControlLabel
                      control={
                        <Checkbox
              ...
              <FormHelperText>
                Select users who can join this workspace
              </FormHelperText>
            </FormControl>
          </div>
        </div>
