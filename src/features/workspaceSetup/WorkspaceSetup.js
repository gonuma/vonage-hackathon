...
import { postFile, deleteFile, fetchAllFiles } from '../../slices/filesSlice'
...
export function WorkspaceSetup({credentials}) {
  ...
  const generateId = async () => {
    await vonageVideo.createSession( async (err, session) => {
      console.log(session)
      const token = await vonageVideo.generateToken(session.sessionId);
      addRoomHandler({token, sessionId: session.sessionId})
    });
  };
...
const nameChangeHandler = (e, workspace) => {
  ...
  const addRoomHandler = async (session) => {
    dispatch(postWorkspace({user: user.selectedUser, session}))
  }
    ...
    return (
      <div className={classes.roomSelector}>
        <List>
          ...
          <Button color="primary" onClick={(e) => generateId()}> Add a new room</Button>
